...
2. **最少文件数**：减少工具调用次数。4 个文件，agent 最多 2-3 次读取
...
.agents/skills/                 ←  规则文件，存在用户知识库中（路径相对于 mindRoot）
  mindos/                          英文版 skill 规则（skills/mindos/SKILL.md 引用）
    rules.md                       主规则（core + tool guide + safety）
    patterns.md                    执行模式与交互规则（按需）
    proactive.md                   主动智能（post-task hooks + 用户自定义 hooks）
    user-rules.md                  用户个性化规则（多类型：偏好/工作流/抑制）
...
1. Read `.agents/skills/mindos/rules.md` — primary rule set.
...
2. Read `.agents/skills/mindos/proactive.md` — post-task hooks.
3. For complex tasks (multi-file routing, retrospectives, reorganization):
   also read `.agents/skills/mindos/patterns.md`.
4. If `.agents/skills/mindos/user-rules.md` exists and is non-empty:
   read it. User rules override default rules on conflict.
5. Execute task following loaded rules.
6. After completion, evaluate proactive hooks. Propose if conditions met.
```

~30 行。无内嵌规则，agent 被迫执行加载协议。

---

## Layer 1 — rules.md（主规则，~100 行）
...
一次读取、一次加载、零 routing。安全规则和核心原则不应该"按需加载"。

---

## Layer 2 — patterns.md（执行模式，~60 行，按需）

当前 SKILL.md 的 Execution Patterns + Interaction Rules。

只在复杂任务时加载。简单的"改个笔记"不需要。

---

## Layer 3 — proactive.md（主动智能）

### 为什么需要主动性

当前 skill 是纯工具——用户说什么做什么。但一个好的知识库助手应该像一个有经验的助理：做完事后知道什么值得跟进。

### 设计：两段式

proactive.md 分两部分：**默认 hooks**（预置）+ **用户自定义 hooks**（用户追加）。
...
每次最多提议 1 项（最高优先级的那个），避免轰炸。

## 纪律

1. 简单操作（改标题、追加一行、读取查询）完成后不提议
2. 每次最多 1 个提议，选优先级最高的
3. 一句话 + 具体目标。用户说 yes 再展开
4. 检查 user-rules.md 中的抑制规则，命中则跳过
...
### 一致性同步（优先级：高）
- 条件：编辑了文件 A，A 被其他文件引用（get_backlinks）
...
预置 6 个 hooks 是通用的，但框架必须开放。用户在 proactive.md 的 `用户自定义 Hooks` 区域追加即可。
...
- 无 `--force`：只创建不存在的文件
- 有 `--force`：重置 rules.md + patterns.md + proactive.md，**保留 user-rules.md**
...
| 场景 | 当前 | 新方案 |
|------|------|--------|
| 简单编辑 | 185 行 | ~150 行（rules + proactive） |
| 复杂多文件 | 185 行 | ~210 行（rules + proactive + patterns） |
| 有个性化规则 | 不可能 | +N 行 user-rules |
| 扩展新能力 | 只能塞进 185 行 | 新增规则文件 |

简单操作 token 持平（proactive 是新能力的开销）。核心收益不是 token 节省，是让 skill 变成一个可演化的系统。

## 副本同步

| 之前 | 之后 |
|------|------|
| 3 处 × 185 行 | 3 处 × ~30 行 loader |
...
1. 拆分 `skills/mindos/SKILL.md` → `rules.md` + `patterns.md`
2. 编写 `proactive.md`（默认 hooks + 自定义区域）
3. 编写 `user-rules.md` 空模板（含四类分区结构）
4. 创建 `templates/skill-rules/en/` 和 `zh/`
5. 改写 `skills/mindos/SKILL.md` 为 thin loader
6. 同步 `skills/mindos-zh/SKILL.md`
7. 同步 `app/data/skills/` 副本

### Phase 2：初始化集成

8. `scripts/setup.js` onboard 增加 skill rules 初始化
9. `bin/cli.js` 增加 `init-skills` 子命令
10. `mindos update` 增加规则版本检测

### Phase 3：验证

11. 新用户 onboard → 规则文件正确创建
12. 已有用户 → 无规则文件时 fallback 到 mindos_bootstrap
13. `mindos init-skills` → 不覆盖 user-rules.md
14. 显式偏好写入 → 下次加载生效
15. proactive hooks → 任务后正确提议，拒绝 3 次自动关闭
16. `npm test` 通过
